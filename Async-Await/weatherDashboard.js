...
/*const cityNames = await fetchCityCoordinates(cities);
        const weather = await fetchWeatherData(cityNames.id);
        const airQuality = await fetchAirQualityData(cityNames.id);

        const cityWeather = await Promise.all(
            cityNames.map(async (city) => {
                const cityWeather = await fetchWeatherData(city.weather);

                return { ...cityNames, weather }
            })
        );

        const cityAirQuality = await Promise.all(
            cityNames.map(async (city) => {
                const cityAir = await fetchAirQualityData(city.airQuality);

                return { ...cityNames, airQuality }
            })
        )
